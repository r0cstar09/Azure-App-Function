...
module.exports = async function (context, myTimer) {
  ...
  if (myTimer?.IsPastDue) context.log.warn("[WARN] Timer is past due");
  ...
  try {
    ...
    context.log(`[INFO] ${shortlisted.length} jobs above threshold (${SCORE_THRESHOLD})`);
    ...
    for (const job of shortlisted) {
      ...
      try {
        ...
        context.log.error(`[FAIL] Generate for ${job.title} @ ${job.company}: ${err.message}`);
      }
    }
